...
def it_handler(self, method, n, m):
    ...
    if method == 'weightSum': self.assertEqual(old.weightSum, new.weightSum)
    if method == 'normalizedResidual':
        self.assertEqual(old.normalizedResidual, new.normalizedResidual)
    if method == 'chiRedMin': self.assertEqual(old.chiRedMin, new.chiRedMin)
    ...
    if method == 'TeErrMin': self.assertEqual(old.TeErrMin, new.TeErrMin)
    if method == 'TeErrMax': self.assertEqual(old.TeErrMax, new.TeErrMax)
    if method == 'ne': self.assertEqual(old.ne, new.ne)
    if method == 'neErrMin': self.assertEqual(old.neErrMin, new.neErrMin)
    if method == 'neErrMax': self.assertEqual(old.neErrMax, new.neErrMax)
